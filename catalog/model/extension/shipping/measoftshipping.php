<?php
class ModelExtensionShippingMeasoftshipping extends Model
{
    public function getQuote($address)
    {
        ...
        if ($result) {
            ...
            $quote_data['measoftshipping'] = array(
                'code'         => 'measoftshipping.measoftshipping',
                'title'        => $this->config->get('measoftshipping_pvz_description')."<input type='hidden' name='ksProductWeight' id='ksProductWeight' value='".$ksProductWeight."' />",
                'cost'         => $cost,
                'tax_class_id' => $this->config->get('measoftshipping_tax_class_id'),
                'sort_order'   => $this->config->get('measoftshipping_sort_order'),
                'text'         => $this->currency->format($this->tax->calculate($cost, $this->config->get('measoftshipping_tax_class_id'), $this->config->get('config_tax')), $this->session->data['currency'])
            );

            $method_data = array(
                'code'       => 'measoftshipping',
                'title'      => $this->config->get('measoftshipping_pvz_title').$interval,
                'quote'      => $quote_data,
                'sort_order' => $this->config->get('measoftshipping_sort_order'),
                'error'      => false
            );
