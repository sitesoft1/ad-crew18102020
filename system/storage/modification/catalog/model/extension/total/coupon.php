...
class ModelExtensionTotalCoupon extends Model {
	public function getCoupon($code) {
		...
		if ($coupon_query->num_rows) {
			...
			if ($coupon_query->row['logged'] && !$this->getOriginalCustomerId()) {
				$status = false;
			}

			if ($this->getOriginalCustomerId()) {
				$coupon_history_query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "coupon_history` ch WHERE ch.coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "' AND ch.customer_id = '" . (int)$this->getOriginalCustomerId() . "'");
	...
	public function confirm($order_info, $order_total) {
  	...
  	public function getOriginalCustomerId() {
		if (!empty($this->session->data['original_customer_id'])) {
			return $this->session->data['original_customer_id'];
		} else {
