        ...
        <div class="panel panel-default">
          ...
          <table class="table">
			...
			<?php } ?>  
				...
				<td class="text-right"><input type="text" name="order_track_no" value="<?php echo $track_no; ?>" /></td>
				<td class="text-center"><button id="button-track_no-add" data-loading-text="<?php echo $text_loading; ?>" data-toggle="tooltip" title="<?php echo $text_save_track_no; ?>" class="btn btn-success btn-xs"><i class="fa fa-plus-circle"></i></button>
			  </tr>
			  <tr>
...
<script type="text/javascript">
	$(document).delegate('#input-order-status', 'change', function() {
		...
		$.ajax({
			url: 'index.php?route=sale/omanager/getStatusComment&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&order_status_id=' + order_status_id,
			dataType: 'json',
			beforeSend: function() {
				$('textarea[name="comment"]').val('Loading... please wait');
			},
			success: function(json) {
				if (json['comment']) {
					$('textarea[name="comment"]').val(json['comment']).focus();
				} else {
					$('textarea[name="comment"]').val('');
				}
			},
...
$(document).delegate('#button-track_no-add', 'click', function() {
	$.ajax({
		url: 'index.php?route=sale/omanager/saveTrackNo&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
		type: 'post',
		dataType: 'json',
		data: 'track_no=' + encodeURIComponent($('input[name=\'order_track_no\']').val()),
		beforeSend: function() {
