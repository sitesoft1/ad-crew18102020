...
// CDEK OCMOD START
      ...
      // CDEK OCMOD END
			...
			*/	
			...
			// Omanager ->
			$omanager = array();
			if ($this->user->hasPermission('access', 'sale/omanager')) {
				$this->load->language('sale/omanager');
				$sale[] = array(
					'name'	   => $this->language->get('text_menu_list'),
					'href'     => $this->url->link('sale/omanager', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
				$sale[] = array(
					'name'	   => $this->language->get('text_menu_settings'),
					'href'     => $this->url->link('sale/omanager/settings', 'token=' . $this->session->data['token'], true),
					'children' => array()
				);
			}
			// -> Omanager
